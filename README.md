...
| `CdkFargateDockerEcsAlb`                                                   | Deploys a simple “Hello World” Node.js web application using a Docker image to AWS ECS Fargate. The stack is optimized for cost with no NAT gateways and minimal public subnet configuration. Demonstrates containerized deployment, serverless compute with ECS, and exposing the service through an Application Load Balancer (ALB) for HTTP access.Resources: VPC (public subnet), ECS Cluster, Fargate Service, Fargate Task, Docker image asset, Application Load Balancer (ALB)                |
